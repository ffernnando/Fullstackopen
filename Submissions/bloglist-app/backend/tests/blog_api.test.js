...
test("POST request successfully create new blog posts", async () => {
  ...
  const hasNewBlogTitle = () => {
    ...
    for (let x of blogsAfter) {
      Object.values(x).includes(newBlog.title)
        ? (contains = true)
        : (contains = false);
    }
    return contains;
...
test('if "likes" property is missing from the request, it will default to 0', async () => {
  ...
  assert.strictEqual(
    blogsAfter.find((blog) => blog.title === newBlog.title).likes,
    0,
  );
});
...
describe("DELETE and UPDATE", () => {
  test("delete a single blog post", async () => {
    ...
    const id = blogs[0].id;
    //console.log("ID: ", id)
    await api.del(`/api/blogs/${id}`).expect(204);
    const blogsAfter = await blogsInDb();
  ...
  test("update blog post's likes", async () => {
    ...
    await api.put(`/api/blogs/${id}`).send({ likes: 500 }).expect(201);
